...
class Categorical(GibbsSampling, MeanField, MeanFieldSVI, MaxLikelihood, MAP):
    ...
    def log_likelihood(self, x, mask=None):
        ...
        out[~nanidx] = np.log(self.weights)[
            list(x[~nanidx])]  # log(0) can happen, no warning
        np.seterr(**err)
        return out

    ### Gibbs sampling

    def resample(self, data=[], mask=None, counts=None):
    ...
    def _get_statistics(self, data, mask=None, K=None):
        data = get_masked_data(data, mask)
    ...
    def _get_weighted_statistics(self, data, weights, mask=None):
        data = get_masked_data(data, mask)
    ...
    def meanfieldupdate(self, data, weights, mask=None):
        data = get_masked_data(data, mask)
        # update
        self._alpha_mf = self.alphav_0 + self._get_weighted_statistics(data,
                                                                       weights)
    ...
    def get_vlb(self):
        ...
        q_entropy = -1 * (
                ...
                + special.gammaln(self._alpha_mf.sum()) - special.gammaln(
            self._alpha_mf).sum())
        p_avgengy = special.gammaln(self.alphav_0.sum()) - special.gammaln(
            self.alphav_0).sum() \
...
class CategoricalAndConcentration(Categorical):
    ...
    def meanfieldupdate(self, *args, **kwargs):  # TODO
        warn(
            'MeanField not implemented for %s; concentration parameter will stay fixed')
        return super(CategoricalAndConcentration, self).meanfieldupdate(*args,
                                                                        **kwargs)
...
class Multinomial(Categorical):
    ...
    def __init__(self, weights=None, alpha_0=None, K=None, alphav_0=None,
                 ...
                 N=1):
        self.N = N
        super(Multinomial, self).__init__(weights, alpha_0, K, alphav_0,
                                          alpha_mf)

    def log_likelihood(self, x):
        assert isinstance(x, np.ndarray) and x.ndim == 2 and x.shape[
            1] == self.K
    ...
    def expected_log_likelihood(self, x=None):
        if x is not None and (
                not x.ndim == 2 or not np.all(x == np.eye(x.shape[0]))):
...
class CRP(GibbsSampling):
    ...
    def rvs(self, customer_counts):
        ...
        assert isinstance(customer_counts, list) or isinstance(customer_counts,
                                                               int)
        ...
        for num in customer_counts:
            ...
            tables = []
            for c in range(num):
                newidx = sample_discrete(
                    np.array(tables + [self.concentration]))
    ...
    def log_likelihood(self, restaurants):
        ...
        for counts in restaurants:
            counts = np.array([c for c in counts if
                               c > 0])  # remove zero counts b/c of gammaln
            K = len(counts)  # number of tables
            N = sum(counts)  # number of customers
            likes.append(K * np.log(self.concentration) + np.sum(
                special.gammaln(counts)) +
    ...
    def _posterior_hypparams(self, sample_numbers, total_num_distinct):
        ...
        if total_num_distinct > 0:
            ...
            svec = np.array(stats.bernoulli.rvs(
                sample_numbers / (sample_numbers + self.concentration)))
            return self.a_0 + total_num_distinct - svec.sum(), (
                        self.b_0 - np.log(wvec).sum())
        else:
...
class GammaCompoundDirichlet(CRP):
    ...
    def resample(self, data=[], niter=50, weighted_cols=None):
        ...
        if size > 0:
            return super(GammaCompoundDirichlet, self).resample(data,
                                                                niter=niter)
        else:
    ...
    def _get_statistics(self, data):
        ...
        if counts.sum() == 0:
            ...
            m = sample_crp_tablecounts(self.concentration, counts,
                                       self.weighted_cols)
        ...
        if counts.sum() == 0:
            ...
            for (i, j), n in np.ndenumerate(counts):
                m += (np.random.rand(n) < self.concentration * self.K *
                      self.weighted_cols[j] \
                      / (np.arange(n) + self.concentration * self.K *
                         self.weighted_cols[j])).sum()
